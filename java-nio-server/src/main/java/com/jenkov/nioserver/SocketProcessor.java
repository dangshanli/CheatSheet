...
public class SocketProcessor implements Runnable {
    ...
    /**
     * 将Queue<Socket>里面的SocketChannel一一取出来，并且注册到Selector
     * @throws IOException
     */
    ...
    private void readFromSocket(SelectionKey key) throws IOException {
        ...
        socket.messageReader.read(socket, this.readByteBuffer);//解析从SocketChannel读取的byte

        List<Message> fullMessages = socket.messageReader.getMessages();
        if(fullMessages.size() > 0){
            for(Message message : fullMessages){
                message.socketId = socket.socketId;
                //处理Message，并将处理结果输出成Message,并保存到MessageQueue:outboundMessageQueue
    ...
    /**
     * 将处理完输入请求后，生成的输入写入SocketChannel
     * @throws IOException
     */
